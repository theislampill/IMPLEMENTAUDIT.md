...
### Canonical return envelope consumer
...
For this audit-implement-return, the governor invokes the same source-owned
consumer with the bound values:
`python -B <skill-dir>/scripts/validate-audit-implement-return.py --input <return.json> --expect-audit-object <audit-object> --expect-proposition-domain <domain> --expect-proposition <proposition> --expect-evidence-id <evidence-id> --expect-evidence-sha256 <sha256> --expect-evidence-kind <kind>`.
Here <skill-dir> is the executing governor directory in either projection.
No authority follows from a structural PASS; the existing semantic judgment and
exact evidence/proposition bindings remain unchanged.
