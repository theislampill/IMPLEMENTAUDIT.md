...
Before the complete admission census, read enough current evidence to choose
exactly one of `NO_ACTION`, `SUPPORTING`, `AMEND`, `DEFER`, or `NEW_RXX`:
...
Allocate an RXX number only after selecting `NEW_RXX`. `NEW_RXX` may allocate
that stable work-order number before the complete admission census, but it is
not an issue number, filing authority, or a substitute for the later census and
reconciliation. `NO_ACTION`, `SUPPORTING`, `AMEND`, and `DEFER` allocate no RXX
