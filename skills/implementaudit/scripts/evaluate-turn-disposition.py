...
def validate_route(route: Any, binding: dict[str, Any]) -> dict[str, Any]:
    # The existing v1 request/record/result contract preserves this optional
    # field. Historical absence is not permission to discard malformed metadata.
    result_keys = set(ROUTE_RESULT_KEYS)
    if isinstance(route, dict) and "presentation" in route:
        result_keys.add("presentation")
    result = exact_object(route, result_keys, "route")
    if "presentation" in result:
        presentation = exact_object(
            result["presentation"], {"parent_holon", "consuming_frontier"}, "route.presentation"
        )
        for key, text in presentation.items():
            if (not isinstance(text, str) or not 0 < len(text) <= 240 or text != text.strip()
                ...
                raise InputError(f"route.presentation has malformed {key}")
    ...
    if decision == "NOT_REQUIRED":
        if not (
            ...
            and result["selected_child"] is None
