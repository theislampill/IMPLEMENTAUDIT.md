...
if [ "${1:-}" = "--mutation-authority" ]; then
  ...
  [ "$step" -gt 0 ] 2>/dev/null || { printf 'validate-phase: mutation authority step must be a positive decimal\n' >&2; exit 1; }
  ...
  "${py[@]}" - "$phase_file" "$step" "$repo_root" "$run_root" <<'PY'
import hashlib,json,os,re,stat,sys
from pathlib import Path
phase_file,step_text,repo_text,run_text=sys.argv[1:]; step=int(step_text)
...
if phase != run/'phases'/f'phase-{step}.md': die('phase path does not equal the run-root phase/step path')
...
if len(pm)!=1 or int(pm[0])!=step: die('Phase header does not equal requested step phase')
...
if not roadmap.is_file() or len(re.findall(rf'(?m)^\|\s*{step}\s*\|',roadmap.read_text(encoding='utf-8')))!=1: die('ROADMAP does not contain exactly one matching phase row')
def pathok(x):
...
try: scope=json.loads(rows[0],object_pairs_hook=dict)
except json.JSONDecodeError: die('Mutation scope is not JSON')
...
try: auth=json.loads(rows[0],object_pairs_hook=dict)
except json.JSONDecodeError: die('mutation-authority is not JSON')
...
binding={'schema':'implementaudit.phase-mutation-authority-binding.v1','claim_id':ids[0],'run_root':run_rel,'phase':step,'step':step,'owner_source':owner,'baseline_ref':baseline,'operation':op,'source':source,'destination':dest,'mutation_scope':scope}
digest=hashlib.sha256(json.dumps(binding,sort_keys=True,separators=(',',':'),ensure_ascii=False).encode()).hexdigest()
print(json.dumps({'schema':'implementaudit.phase-mutation-authority.v1','phase':step,'step':step,'run_root':run_rel,'operation':op,'source':source,'destination':dest,'mutation_scope':scope,'authority_binding_sha256':digest},separators=(',',':'),ensure_ascii=False))
PY
  exit $?
fi
