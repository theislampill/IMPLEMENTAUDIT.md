...
if [ "${1:-}" = --automatic-effects ]; then
  ...
  ensure_python
  "${py_cmd[@]}" - "$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)/closure_automatic_effects.py" "$effect_root" "$effect_event" "$effect_ref" "$effect_plan" <<'PY'
import sys
...
def _load_closure_effects():
    ...
    _path = _os.path.join(_os.path.dirname(_os.path.abspath(_sys.argv[1])), 'closure_automatic_effects.py')
    _fd = None
    try:
        ...
        if (_stat.S_ISLNK(_before.st_mode) or not _stat.S_ISREG(_before.st_mode)
            ...
            raise ImportError("closure_automatic_effects.py: source is not a non-aliased regular file")
        ...
        if (not _stat.S_ISREG(_info.st_mode) or (_before.st_dev, _before.st_ino) != (_info.st_dev, _info.st_ino)):
            raise ImportError("closure_automatic_effects.py: source identity changed while opening")
        with _os.fdopen(_fd, "rb") as _stream:
            _fd = None
            _source = _stream.read(_CLOSURE_EFFECTS_BYTES + 1)
    except OSError as _exc:
        raise ImportError("closure_automatic_effects.py: source unavailable") from _exc
    finally:
        if _fd is not None:
            _os.close(_fd)
    if len(_source) != _CLOSURE_EFFECTS_BYTES or _hashlib.sha256(_source).hexdigest() != _CLOSURE_EFFECTS_SHA256:
        raise ImportError("closure_automatic_effects.py: source identity mismatch")
    _module = _types.ModuleType(__name__ + ".closure_effects")
    ...
    try:
        ...
        raise
    return _module
_closure_effects = _load_closure_effects()

raise SystemExit(_closure_effects.main(sys.argv[2:]))
PY
  exit $?
fi
...
for claim_id, claim in claims.items():
    ...
    if publication_kind == "hosted-release-asset":
        if set(identity) != hosted_identity or identity.get("publication-kind") != publication_kind:
            ...
            die(f"claim {claim_id}: hosted release asset requires checker-captured public evidence and a qualification record")
        ...
        if not sha_re.fullmatch(qualification_sha) or hashlib.sha256(qualification_data).hexdigest() != qualification_sha:
            die(f"claim {claim_id}: qualification record SHA-256 mismatch")
        try:
            qualification_payload = json.loads(
                qualification_data.decode("utf-8"),
        ...
        if (
            type(qualification_payload) is not dict
            or type(qualification_payload.get("package_size")) is not int
            or qualification_payload != expected_qualification
        ):
            die(f"claim {claim_id}: qualification record does not bind commit/tree/package identity")
        ...
        if captured_asset != package_data:
            die(f"claim {claim_id}: captured public asset bytes do not match qualified package")
        ...
        try:
            payload = json.loads(
                readback_data.decode("utf-8"), object_pairs_hook=unique_object,
                parse_constant=reject_constant,
            )
        except (UnicodeDecodeError, json.JSONDecodeError, ValueError):
            die(f"claim {claim_id}: public readback is not valid unique-key UTF-8 JSON")
        public_keys = (
