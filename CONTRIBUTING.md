...
| Plugin/runtime metadata | `.codex-plugin/plugin.json` and `.claude-plugin/plugin.json` preserve equal shared package semantics and both point to `./skills/`; host-owned extensions such as Codex `interface` metadata need not be copied into the other host manifest. `.claude-plugin/marketplace.json` remains Claude marketplace metadata. |
