...
def _load_host_command_policy():
    path = os.path.join(HERE, "lib", "host_command_policy.py")
    descriptor = None
    try:
        ...
        if not stat.S_ISREG(info.st_mode):
            raise ImportError("host command policy is not a regular source file")
        with os.fdopen(descriptor, "rb") as stream:
            descriptor = None
            source = stream.read(_HOST_COMMAND_POLICY_BYTES + 1)
    except OSError as exc:
        raise ImportError("host command policy source unavailable") from exc
    finally:
        if descriptor is not None:
            os.close(descriptor)
    if (len(source) != _HOST_COMMAND_POLICY_BYTES or
            hashlib.sha256(source).hexdigest() != _HOST_COMMAND_POLICY_SHA256):
        raise ImportError("host command policy source identity mismatch")
    module = types.ModuleType(__name__ + "._host_command_policy")
    module.__file__ = path
    exec(compile(source, path, "exec"), module.__dict__)
    sys.modules[module.__name__] = module
    return module.HostCommandPolicy


HostCommandPolicy = _load_host_command_policy()
...
class _BaseAdapter(HostCommandPolicy):
    name = "base"
    version = "3"
    timeout_s = 1800
    ...
    def _classify_command_target(self, record, expected, repo):
        """Compatibility bridge: the adapter supplies attested host facts."""
        return self.classify_command_target(
            record, expected, repo, self._host_read_profile(repo))
