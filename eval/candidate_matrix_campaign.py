#!/usr/bin/env python3
"""Serialized, create-once driver for the frozen candidate matrix campaign."""
...
def _validate_official_verdict(verdict, fixture=None, *, packet=None,
    ...
    if fixture is None:
        fixture = _read_object(
            pathlib.Path(__file__).resolve().parent / "fixtures" / "B0" /
            "fixture.json", "B0 fixture")
    ...
    if production and packet is not None and mission is not None:
        ...
        expected = {
            "run_id": (f"attempt-{mission['index']:03d}-L-"
                       f"{mission['fixture']}"),
            ...
            "product_tree": candidate["tree"],
            "installed_payload_sha256": candidate["payload_sha256"],
...
def _fixture_for_packet(repo_root, packet, mission):
    identity = next(
        row for row in packet["fixtures"]
        if row["id"] == mission["fixture"])
    path = pathlib.Path(repo_root) / pathlib.PurePosixPath(identity["path"])
    raw = contract.read_custodied_bytes(
...
def validate_runtime_identities(packet, *, candidate_checkout):
    ...
    checkout = pathlib.Path(candidate_checkout).resolve()
    expected = packet["candidate"]
    if _git(checkout, "rev-parse", "HEAD") != expected["commit"]:
        raise ValueError("candidate commit identity drift")
    ...
    if _git(checkout, "rev-parse", "HEAD:skills/implementaudit") != \
            expected["skill_tree"]:
        raise ValueError("candidate skill tree identity drift")
    payload = checkout / "skills" / "implementaudit"
    if adapters.payload_hash(str(payload)) != expected["payload_sha256"]:
        raise ValueError("candidate payload identity drift")
    executable = packet["configuration"]["executable"]
    path = contract.resolve_external_file(
        executable["path"], "configuration L executable")
    if _sha256_file(path) != executable["sha256"]:
        raise ValueError("configuration L executable identity drift")
    ...
    if proc.returncode or executable["version"] not in observed:
        raise ValueError("configuration L executable version drift")
...
class MissionContext:
    def __init__(self, **values):
        ...
        self.expected_model = self.packet["configuration"][
...
class CampaignDriver:
    ...
    def __init__(self, *, packet_path, repo_root, campaign_root,
        ...
        self.attestation = pathlib.Path(attestation) if attestation else None
    ...
    def _load_launch_context(self):
        ...
        if (self.attestation is None or type(attestation) is not str or
                self.attestation.resolve() !=
    ...
    def _load_packet(self):
        raw = contract.read_custodied_bytes(
            self.packet_path, "candidate matrix freeze input", root=self.packet_path.parent)
    ...
    def _ensure_campaign(self, raw, packet_sha256, packet):
        ...
        if not self.campaign_root.exists():
            ...
            _write_new_json(manifest, {
                "schema": "implementaudit-candidate-matrix-luna-campaign-custody-v1",
    ...
    def _next_mission(
        ...
        if (stage_artifacts & {p.name for p in self.campaign_root.iterdir()} and
                len(actual) != len(packet["cells"])):
            raise ValueError("Luna stage artifact exists before fourteen terminals")
        ...
        for mission in packet["cells"]:
            ...
            if terminal.get("official_verdict_sha256") is not None:
                _verify_official_verdict(
                    root, terminal,
                    _fixture_for_packet(self.repo_root, packet, mission),
        ...
        raise ValueError(
            "campaign already contains all fourteen Luna attempts; "
    ...
    def _load_host_attestation(self, mission, packet):
        ...
        expected = packet["configuration"]["host_attestation"]
    ...
    def run_next(self):
        ...
        completed_prefix = [
            self._attempt_name(row) for row in packet["cells"]
    ...
    def _validate_independent_luna_result(self, value, packet, packet_sha256,
        ...
        if (value["schema"] !=
                "implementaudit-candidate-matrix-luna-independent-rederivation-v1" or
                ...
                len(value["cells"]) != 14):
            raise ValueError("independent Luna result is not accepted 14/14")
        self._result_claims(value["claims"], "independent Luna result")
        for expected, observed in zip(summaries, value["cells"]):
            if type(observed) is not dict or set(observed) != \
                    INDEPENDENT_PASS_ROW_FIELDS:
                raise ValueError("independent Luna cell row schema invalid")
    ...
    def _luna_summaries(self, packet, packet_sha256, rows):
        ...
        summaries = []
        for mission, row in zip(packet["cells"], rows):
            fixture = _fixture_for_packet(self.repo_root, packet, mission)
            ...
            if (terminal["overall_status"] != "PASS" or
                    ...
                    terminal["resolved_model"] != packet["configuration"][
                        "model_resolved_required"] or
    ...
    def finalize_luna_stage(self):
        result_path = self.campaign_root / "candidate-matrix-luna-result.json"
    ...
    def _execute_formal_host(self, context):
        ...
        config_name = "L"
        config = context.packet["configuration"]
        product = context.candidate_checkout
        ...
        common = {
            ...
            "product_expected_rev": context.packet["candidate"]["commit"],
        ...
        result = adapter.run_mission(
            mission["fixture"], str(custody), run_id,
...
def main(argv=None):
    ...
    if args.operation == "run-next":
        ...
        if terminal.get("schema") == \
                "implementaudit-candidate-matrix-campaign-andon-v1":
            print(json.dumps({
                "mission_index": terminal["mission_index"],
                "overall_status": "ERROR",
                "stop_reason": terminal["stop_reason"],
            }, sort_keys=True))
            return 2
    ...
    print(json.dumps({
        ...
        "cell_count": result["cell_count"]}, sort_keys=True))
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"CANDIDATE_MATRIX-CAMPAIGN-REFUSED: {type(exc).__name__}", file=sys.stderr)
        raise SystemExit(2)
