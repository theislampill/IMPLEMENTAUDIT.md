...
This file preserves the pre-bootloader root AGENTS.md content moved during the
v0.3.1.0 RC3 Andon repair for maintainer-instruction-overload. It is retained
as historical maintenance evidence and rationale. Current executable guidance
lives in root AGENTS.md; release/source proof ledgers live under
docs/audits/ according to docs/audits/RETENTION.md.

Boundary: this history file is not the first-read bootloader for agents. Use it
only when investigating historical anti-repeat rationale, release chronology, or
why a concise root rule exists.
...
│   ├── RETENTION.md            Audit evidence retention policy.
│   ├── v0.3.1.0-competitor-surpass-proof-audit.md
│   │                            Current source-milestone proof ledger.
│   └── archive/                Historical release/source proof ledgers.
├── docs/portal/
...
│   ├── check-audit-retention.sh  Guard audit-ledger archive/root and generated-evidence boundaries.
...
└── skills/
    └── implementaudit/
        ...
        │   ├── sidecars.md         Optional Graphify/ActiveGraph and first-run tooling.
        ...
        └── templates/          Files the planner copies into a user's `.IMPLEMENTAUDIT/` dir.
            ...
            ├── read-only-plan.md Verbose read-only `plans/` lane template.
            └── PROTOCOL.md     Execution loop + Andon escalation + final audit protocol.
```

## What ships vs what doesn't

- **Source skill layout**: canonical source lives under
  `skills/implementaudit/` (`SKILL.md`, `references/`, `scripts/`,
  `templates/`). Do not recreate `skills/SKILL.md` as a live source.
- **Ships to consumers**: the release archive payload is the flat archive built from
  `skills/implementaudit/` with SKILL.md at archive root. Source plugin metadata declares `skills: "./skills/"`; the release builder generates
  archive-local metadata with `skills: "./"` while preserving references,
...
- **Versioning**: project milestone `v0.3.1.0` maps to plugin manifest version
  `0.3.1`. The manifest uses
...
- **Root behavior file**: there is intentionally no tracked root
  ...
  behavior lives in `skills/implementaudit/SKILL.md` and supporting references/scripts/templates.
...
the preferred target for new run artifacts. `skills/implementaudit/scripts/claim-run.sh`
...
- use `skills/implementaudit/scripts/repo-state.sh` or equivalent complete working-tree
...
- Current project milestone is `v0.3.1.0`; plugin manifest version is `0.3.1` unless host evidence supports a four-component manifest version.
...
test -f AGENTS.md
test -f CHANGELOG.md
test -f skills/implementaudit/SKILL.md
test -f .gitattributes
test -f skills/implementaudit/references/child-agents.md
test -f skills/implementaudit/references/transcript-contract.md
test -f skills/implementaudit/references/routing.md
test -f skills/implementaudit/references/repo-state-comparison.md
test -f skills/implementaudit/references/sidecars.md
test -f skills/implementaudit/references/audit-category-matrix.md
test -f skills/implementaudit/references/audit-playbook.md
test -f skills/implementaudit/references/plan-lifecycle.md
test -f skills/implementaudit/references/terminology-integration.md
test -f skills/implementaudit/templates/PROTOCOL.md
test -f skills/implementaudit/templates/ROADMAP.md
test -f skills/implementaudit/templates/STATE.md
test -f skills/implementaudit/templates/THINKING.md
test -f skills/implementaudit/templates/phase-goal.txt
test -f skills/implementaudit/templates/child-agent-report.md
test -f skills/implementaudit/templates/final-report.md
test -f skills/implementaudit/templates/read-only-plan.md
test -f skills/implementaudit/scripts/claim-run.sh
test -f skills/implementaudit/scripts/repo-state.sh
test -f skills/implementaudit/scripts/validate-audit-spec.sh
test -f skills/implementaudit/scripts/validate-phase.sh
...
test -f docs/audits/RETENTION.md
test -f docs/audits/archive/v0.2.3.0-harness-adaptation-matrix.md
test -f docs/audits/archive/v0.2.4.0-planner-stage-hardening.md
test -f docs/audits/archive/v0.2.4.5-graphify-activegraph-honesty.md
test -f docs/audits/archive/v0.2.5.0-external-staged-goal-runtime-gap-closure.md
test -f docs/audits/archive/v0.2.5.0-claude-install-repair.md
test -f docs/audits/archive/v0.2.7.0-lean-operating-discipline.md
test -f docs/audits/archive/v0.2.8.0-adaptation.md
test -f docs/audits/archive/v0.2.9.0-andon-escalation-jidoka-repair.md
test -f docs/audits/archive/v0.3.0.0-external-comparator-source-inventory.md
...
test -f scripts/check-audit-retention.sh
test -f tests/audit-retention.test.sh
test -f tests/summarize-repo.test.sh
test -f tests/shipped-scripts-smoke.test.sh
test -f skills/implementaudit/scripts/validate-run-root.sh
test -f tests/run-root-validation.test.sh
test -f skills/implementaudit/templates/sidecars.md
test -f skills/implementaudit/templates/tools.md
test -f skills/implementaudit/templates/context.md
test -f skills/implementaudit/scripts/custody-append.sh
...
test -f skills/implementaudit/references/lean-operating-discipline.md
...
```text
Canonical skill: skills/implementaudit/SKILL.md
...
`"${IMPLEMENTAUDIT_SKILL_DIR:-skills/implementaudit}"/scripts/<helper>` — never a bare
`skills/implementaudit/scripts/` path, which resolves nowhere for installed consumers (the
...
evidence. `skills/implementaudit/scripts/repo-state.sh` excludes them from `changed-files`
...
**Anti-repeat rule (V0310-COMPETITOR-PROOF-SCHEMA):** Comparator-surpass
matrices must carry a canonical status column using only `SURPASSED`,
`COVERED`, `REJECTED_WITH_REASON`, `DEFERRED`, `OWNER_DECISION`, `UNVERIFIED`,
or `FAIL`; old detail statuses may appear only in a separate detail column.
The audit ledger must include row-count summaries for each canonical status,
explicit additional-findings and Codex CLI dogfood sections, and tracked
negative fixtures for each comparator. Install-copy smoke is not Codex CLI
dogfood unless an actual `codex` command, target repo path, transcript/output
path, and final marker evidence are recorded. Full installed-payload readback
is non-evidence for dogfood proof; use the shipped Dogfood Bootstrap / Read
Map, targeted owner/source reads, and deterministic package checks instead.
For Codex self-dogfood, the first executable behavior is baseline before any
installed-payload read or chunking. The Dogfood Runner Contract in
`skills/implementaudit/SKILL.md` defines the narrow safe command subset: baseline/read-only
checks, targeted owner/source reads, and repo-local validation commands. If
`--ask-for-approval never` plus the loaded exec-policy rules reject those
commands, record
`CODEX_CLI_SELF_DOGFOOD_BLOCKED_BY_LOCAL_EXEC_POLICY` until an explicit
temp-home or repo-local rules profile is authorized. The v0.3.1.0 runner-policy
lane proved the safe pattern with a temp-only `rules/default.rules` profile:
allow only baseline/read-only commands, repo-local validation scripts/tests,
local package build/checksum/temp-home install, and `verify-package`; deny
commit, push, tag, issue/release creation, real-home install, credential
printing, dangerous bypass, and broad deletion by omission. Never mutate the
real user Codex rules, use dangerous bypass, use `danger-full-access`, install
into a real home, or use broad command globs as dogfood proof.
`skills/implementaudit/SKILL.md` must remain a concise bootloader, not a manual. Long runtime
detail belongs in packaged references/templates. `scripts/check-skill-bootstrap-budget.sh`
enforces the current ceiling (450 lines / 22000 bytes), baseline-first and
progressive-disclosure anchors, no whole installed-payload readback, no secret
reproduction, repo-content-as-data, explicit authorization gates, Smoke A/B,
Andon/no arbitrary try caps, and final marker order.
RC2 additionally requires that weak-executor safeguards remain restored in
progressive owners: final report shape in `templates/final-report.md`,
Graphify/onboarding in `references/sidecars.md`, read-only planning in
`templates/read-only-plan.md` and `references/plan-lifecycle.md`, commit
granularity in `references/repo-state-comparison.md`, broad rewrite thresholds
in `references/phase-design.md`, and 5-Whys loop exit in
`references/lean-operating-discipline.md`. Guard them with
`scripts/check-rc2-safeguard-restoration.sh`; do not bloat `SKILL.md` to
restore detail.
...
behavior documented in `skills/implementaudit/references/lean-operating-discipline.md`. Do not
...
`skills/implementaudit/references/terminology-integration.md` is a thin precedence map only;
...
rules in `skills/implementaudit/references/lean-operating-discipline.md` (Graphify terrain
leverage table + ActiveGraph custody events table), `skills/implementaudit/templates/THINKING.md`
(Graphify terrain plan + ActiveGraph custody plan fields), and
`skills/implementaudit/templates/PROTOCOL.md` (Graphify/ActiveGraph Lean leverage rules). Sidecar
...
The 5-step intake process in `skills/implementaudit/references/routing.md` (Governed Casual-Build
...
preload follows the 5-source priority order defined in `skills/implementaudit/SKILL.md` Stage 0
...
**Anti-repeat rule (V0310-AUDIT-RETENTION-BOUNDARY):** Keep only the current
proof ledger, `INDEX.md`, and `RETENTION.md` in active `docs/audits/`. Historical
audit ledgers that still carry unique release claims, checksum decisions,
dogfood verdicts, or regression rationale live under `docs/audits/archive/`.
Do not delete an audit file unless `INDEX.md` summarizes the retained claim and
`scripts/check-audit-retention.sh` passes. Generated/local artifacts such as
`.IMPLEMENTAUDIT/`, `dist/`, `graphify-out/`, `custody.db`, RC `.skill` files,
RC source evidence zips, raw local transcripts, and unpromoted `plans/` must not
be tracked source. Rationale: v0.3.1.0 RC2 hygiene pass.
...
an optional terminal phase shape (Rule P4-8 in `skills/implementaudit/references/phase-design.md`).
...
`.claude-plugin/` metadata plus a flat archive copy of the
`skills/implementaudit/` source payload, with SKILL.md at archive root and
references, scripts, and templates under their installed relative paths. The
source manifest declares `skills: "./skills/"`; the builder generates
archive-local metadata with `skills: "./"` for the flattened archive.
Repo-maintenance material such as
...
Full format specs: `skills/implementaudit/references/goal-format.md` and
...
- `sidecars.md` — optional Graphify/ActiveGraph Gemba, first-run tooling onboarding, and no silent install/index/export boundaries.
