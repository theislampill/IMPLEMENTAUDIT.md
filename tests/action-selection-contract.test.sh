...
# 58. The verifier plan must not stay narrow when its semantic owner, consumer,
# or effect is omitted from the plan.
for observation in semantic_owner consumer effect; do
  reset_sandbox
  "${py_cmd[@]}" - \
...
payload = json.loads(path.read_text(encoding="utf-8"))
case = next(x for x in payload["cases"] if x["id"] == "R0022-C147-trivial-local-verifier-cheap-path")
case["observations"][observation] = False
path.write_text(json.dumps(payload), encoding="utf-8")
PY
  expect_fail "under-scoped verifier plan accepted with omitted $observation"
done

# 59. A known transitive, package, or public consumer cannot be relabelled as
# an ordinary local verifier plan.
for case_id in R0022-C150-runtime-consumer-verifier-plan R0022-C151-package-consumer-verifier-plan R0022-C153-public-consumer-verifier-plan R0022-C156-external-consumer-verifier-plan; do
  reset_sandbox
  "${py_cmd[@]}" - \
    "$tmp_root/fixtures/audit-action-selection/engineering-value-cases.json" \
    "$case_id" <<'PY'
...
# 60. Unknown ownership, registry/release ambiguity, and stale public
# projections remain conservative rather than becoming a cheaper local plan.
for case_id in R0022-C152-registry-change-escalates R0022-C154-unknown-owner-escalates R0022-C155-stale-projection-escalates; do
  reset_sandbox
  "${py_cmd[@]}" - \
    "$tmp_root/fixtures/audit-action-selection/engineering-value-cases.json" \
    "$case_id" <<'PY'
