...
# A secondary Status/DONE-shaped row must not override the canonical current
# phase Status row when deciding whether DONE templates are terminal.
secondary_status_open="$tmp/secondary-status-open"
cp -r "$tmp/good/." "$secondary_status_open/"
printf '\n| Status | DONE |\n' >> "$secondary_status_open/STATE.md"
if ! secondary_status_output="$(bash "$helper" "$secondary_status_open" 2>&1)"; then
  printf 'run-root-validation.test: open current Status must remain nonterminal with a secondary DONE row\n' >&2
  printf '%s\n' "$secondary_status_output" >&2
  exit 1
fi
if grep -Fq 'planning artifact remains an unfilled template at DONE:' <<<"$secondary_status_output"; then
  printf 'run-root-validation.test: open current Status emitted a false terminal-template error\n' >&2
  printf '%s\n' "$secondary_status_output" >&2
  exit 1
fi
...
if MSYS=winsymlinks:nativestrict ln -s packet-target.md \
  "$review_packet_symlink/reviews/packet.md" 2>/dev/null; then
  [ -L "$review_packet_symlink/reviews/packet.md" ] || {
    printf 'run-root-validation.test: native packet link creation did not yield a symlink\n' >&2
    exit 1
  }
...
if MSYS=winsymlinks:nativestrict ln -s \
  "$tmp/claim-alias-target" "$tmp/claim-alias-component" 2>/dev/null; then
  [ -L "$tmp/claim-alias-component" ] || {
    printf 'run-root-validation.test: native claim-alias link creation did not yield a symlink\n' >&2
    exit 1
  }
...
if MSYS=winsymlinks:nativestrict ln -s "$claim_root" \
  "$tmp/claim-only-link" 2>/dev/null; then
  [ -L "$tmp/claim-only-link" ] || {
    printf 'run-root-validation.test: native claim-root link creation did not yield a symlink\n' >&2
    exit 1
  }
