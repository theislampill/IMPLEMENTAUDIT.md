...
run_receiver_contract() {
"${py_cmd[@]}" - "$repo_root" "$tmp" <<'PY_RECEIVER'
import hashlib, json, pathlib, shlex, subprocess, sys

root=pathlib.Path(sys.argv[1]).resolve()
out=pathlib.Path(sys.argv[2]).resolve()
doc=root/'skills/implementaudit/references/operational-evidence.md'
helper=root/'skills/implementaudit/scripts/operational-evidence.py'
fixture=root/'fixtures/operational-evidence/valid-minimal.json'
doc_bytes=doc.read_bytes()
raw=doc_bytes.decode('utf-8')
start='<!-- operational-evidence-explicit-record-commands:start -->'
end='<!-- operational-evidence-explicit-record-commands:end -->'
if raw.count(start)!=1 or raw.count(end)!=1:
    raise SystemExit('OE_RECEIVER_ROUTE_ABSENT')
section=raw.split(start,1)[1].split(end,1)[0]
commands=[shlex.split(line) for line in section.splitlines() if line.startswith('python ')]
expected_commands=[['python','-B','$IMPLEMENTAUDIT_SKILL_DIR/scripts/operational-evidence.py',mode,'$RECORD'] for mode in ['validate','canonicalize']]
if commands!=expected_commands:
    raise SystemExit('OE_RECEIVER_COMMAND_BINDING_MISMATCH')
for required in ['standalone-diagnostic','no automatic or default invocation','Structural validation is not currentness','There is no `build` or `refresh` CLI command.']:
    if required not in raw:raise SystemExit('OE_RECEIVER_BOUNDARY_MISSING: '+required)
helper_before=hashlib.sha256(helper.read_bytes()).hexdigest()
schema=root/'skills/implementaudit/references/operational-evidence-schema.json'
schema_before=hashlib.sha256(schema.read_bytes()).hexdigest()
fixture_before=hashlib.sha256(fixture.read_bytes()).hexdigest()
results=[]
def invoke(name,args,expected_rc):
    # argv comes from the owner command or a closed invalid-argument control.
    argv=[sys.executable,'-B',str(helper),*args]
    p=subprocess.run(argv,cwd=root,stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=10)
    if p.returncode!=expected_rc:
        raise SystemExit('OE_RECEIVER_EXIT: '+name+': '+str(p.returncode))
...
for args in commands:
    # Only the exact validated template can dispatch. Never shell-evaluate docs.
    actual.append(invoke(args[3],[args[3],str(fixture)],0))
receipt=json.loads(actual[0].stdout);record=json.loads(actual[1].stdout)
if receipt.get('schema')!='implementaudit-operational-evidence-validation-v1':raise SystemExit('OE_RECEIVER_WRONG_RECEIPT')
if record!=json.loads(fixture.read_bytes()):raise SystemExit('OE_RECEIVER_RECORD_CHANGED')
if hashlib.sha256(actual[1].stdout).hexdigest()!=receipt['canonical_sha256']:raise SystemExit('OE_RECEIVER_CANONICAL_DIGEST')
for name,args in [('absent-build',['build',str(fixture)]),('absent-refresh',['refresh']),('unknown',['not-a-command']),('missing-input',['validate']),('native-override',['status','--repo-root',str(root)])]:
    p=invoke(name,args,2)
    if p.stdout or b'usage:' not in p.stderr:raise SystemExit('OE_RECEIVER_PARSER_REFUSAL: '+name)
malformed=root/'fixtures/operational-evidence/duplicate-key.json'
p=invoke('invalid-record',['validate',str(malformed)],2)
error=json.loads(p.stderr)
if error.get('schema')!='implementaudit-operational-evidence-error-v1' or p.stdout:raise SystemExit('OE_RECEIVER_INVALID_RECORD')
if hashlib.sha256(helper.read_bytes()).hexdigest()!=helper_before:raise SystemExit('OE_RECEIVER_HELPER_MUTATED')
if hashlib.sha256(schema.read_bytes()).hexdigest()!=schema_before:raise SystemExit('OE_RECEIVER_SCHEMA_MUTATED')
if hashlib.sha256(fixture.read_bytes()).hexdigest()!=fixture_before:raise SystemExit('OE_RECEIVER_INPUT_MUTATED')
print(json.dumps({'schema':'ordinary.r38-c11-receiver-probe.v1','cases':results,'owner_document_sha256':hashlib.sha256(doc_bytes).hexdigest(),'helper_sha256':helper_before,'schema_sha256':schema_before,'input_sha256':fixture_before,'actual_pure_CLI_dispatches':2,'argument_refusals':5,'invalid_record_refusals':1,'native_collection_invoked':False,'runtime_query_export_invoked':False,'model_invoked':False,'source_qualification':False,'result':'NONQUALIFYING_RECEIVER_PROBE_PASS'},sort_keys=True))
PY_RECEIVER
}

if [ "${1:-}" = "--receiver-probe" ]; then
  run_receiver_contract
  exit 0
fi
...
if cases != {
        ...
        "causal_cases": [f"C07-R{i:04X}" for i in range(1, 19)],
...
if missing:
    raise SystemExit(f"C07-R0001 status/query/why interfaces are absent: {missing}")
...
if (status["schema"] != "implementaudit.operational-evidence-status.v1" or
        ...
        status["establishes"]):
    raise SystemExit("C07-R0002/R0005 status lost six-family currentness or authority")
...
if ([row["record"]["id"] for row in current_view["rows"]] != [
    ...
    raise SystemExit("C07-R0003 CURRENT-only view hid its omitted-state census")
...
if len(all_view["rows"]) != 3 or not any(
    ...
    raise SystemExit("C07-R0005 query normalized a non-current fact to success")
...
if evidence.canonical_json_v1(evidence.query_family(snapshot, "EVIDENCE")) != \
    ...
    raise SystemExit("C07-R0004 query bytes depend on input ordering")
...
if ([row["id"] for row in why["chain"]] != ["evidence", "failure", "release"] or
    ...
    raise SystemExit("C07-R0006 why chain is not deterministic or lost contrary evidence")
missing_why = evidence.explain_history_why_v1(snapshot, "absent")
if missing_why["status"] != "UNKNOWN" or missing_why["chain"]:
    raise SystemExit("C07-R0006 missing why edge invented a cause")
...
try:
    ...
    if exc.code != "OE_WHY_CYCLE":
        raise SystemExit(f"C07-R0006 wrong why-cycle discriminator: {exc.code}")
else:
    raise SystemExit("C07-R0006 malformed why cycle was accepted")
...
try:
    ...
    raise SystemExit("C07-C01 discriminator is not rejected by canonical R0027")
try:
...
if (selection_calls[:4] != [
    ...
    raise SystemExit("C07-C01 exact-current positive skipped a canonical R0027 binding leg")
...
if ([row["event_id"] for row in result["rows"]] != [target] or
        ...
        result["establishes"]):
    raise SystemExit("C07-R000C/R000F exact query hydrated unrelated history or gained authority")
...
def corrupt_referenced_segment(manifest_value, row):
    if row["event_id"] == target:
        return b"{}"
    raise SystemExit("C07-R000D exact query attempted to hydrate unrelated history")
...
try:
    ...
    if exc.code != "OE_QUERY_SEGMENT_INVALID":
        raise SystemExit(f"C07-R000D referenced corruption returned {exc.code}")
else:
    raise SystemExit("C07-R000D referenced corrupt segment was accepted")
...
try:
    ...
    if exc.code != "OE_QUERY_MANIFEST_INVALID":
        raise SystemExit(f"C07-R000D foreign predecessor returned {exc.code}")
else:
    raise SystemExit("C07-R000D foreign predecessor custody was accepted")
finally:
    selected_current[0] = new_manifest
...
for filters in ({}, {"event_ids": []}, {"event_ids": [True]},
    ...
    try:
        ...
        if exc.code != "OE_QUERY_FILTER_INVALID":
            raise SystemExit(f"C07-R0007 wrong filter discriminator: {exc.code}")
    else:
        raise SystemExit("C07-R0007 empty/malformed filter was accepted")
for bad in (0, -1, True, 1.5):
    try:
        ...
        if exc.code != "OE_QUERY_BOUND_INVALID":
            raise SystemExit(f"C07-R0007 wrong bound discriminator: {exc.code}")
    else:
        raise SystemExit("C07-R0007 non-positive/non-integer bound was accepted")
...
if (overflow["code"] != "OE_QUERY_REQUIRES_BOUNDED_REVIEW" or
    ...
    raise SystemExit("C07-R0008/R000B overflow made a negative or complete claim")
...
if (len(byte_overflow["rows"]) != 1 or not byte_overflow["truncated"] or
    ...
    raise SystemExit("C07-R0008 byte overflow lost bounded coverage or next cursor")
...
if evidence.normalize_history_filters_v1(route_request) != {"event_ids": [target]}:
    raise SystemExit("C07-R000C exact HC-H2B request did not normalize")
loaded.clear()
...
if ([row["event_id"] for row in route_result["rows"]] != [target] or
    ...
    raise SystemExit("C07-R000C exact HC-H2B request was not selectively executed")
for key, value in (
    ...
    try:
        ...
        if exc.code != "OE_QUERY_FILTER_INVALID":
            raise SystemExit(f"C07-R000C malformed route request returned {exc.code}")
    else:
        raise SystemExit("C07-R000C malformed route request was accepted")

parser = evidence.build_cli_parser_v1()
if parser.parse_args(["status"]).command != "status":
    raise SystemExit("C07-R0001 status grammar is absent")
...
if query_args.command != "query" or query_args.family != "CODE":
    raise SystemExit("C07-R0001 query grammar is absent")
if parser.parse_args(["why", "evidence"]).record_id != "evidence":
    raise SystemExit("C07-R0001 why grammar is absent")
effects = []
...
if effects:
    raise SystemExit(f"C07-R0010 pure readers crossed an effect boundary: {effects}")
...
if any(action.dest in ("repository", "run_root", "snapshot_root", "manifest")
    ...
    raise SystemExit("C07-R0010 CLI accepted caller-supplied authority")
PY
}
...
fixture_manifest = json.loads(
    ...
    .read_text(encoding="utf-8"))
if (fixture_manifest.get("causal_cases") != [f"C06-R{i:04X}" for i in range(1, 16)] or
...
if list(inspect.signature(snapshot_module.publish_current_snapshot).parameters):
    raise SystemExit("C06-R0009 publisher accepts caller-supplied authority")
...
try:
    ...
    if str(exc) != "OE_R0038_SNAPSHOT_NOT_PUBLISHED":
        raise SystemExit(f"C06-R000A pre-publication consumer returned {exc}")
else:
    raise SystemExit("C06-R000A consumer found a snapshot before publication")

original_external_get = snapshot_module._native_external_get
def forbidden_external_get(*args, **kwargs):
    raise SystemExit("C06-R000F snapshot publisher attempted network")
...
if set(manifest) != {
    ...
    raise SystemExit("C06 manifest key set drifted from the frozen R0027 consumer")
if set(current) != {
        "schema_version", "snapshot_id", "manifest_sha256", "source_pointer_oid"}:
    raise SystemExit("C06 CURRENT key set drifted from the frozen R0027 consumer")
...
if (payload.get("aggregate") != "DEGRADED" or
    ...
    raise SystemExit("C06-R0005 DEGRADED snapshot lost its complete missing census")
...
for key in ("snapshot_id", "input_raw", "payload_raw", "manifest_raw", "current_raw"):
    if original_material[key] != variant_material[key]:
        raise SystemExit(f"C06-R0004 volatile absolute envelope changed {key}")
...
if owner_manifest != {"entries": manifest["source_evidence_entries"]}:
    raise SystemExit("C06-R000A unchanged R0027 consumer did not resolve exact entries")
...
if rotation.load_immutable_r0038_snapshot_for_evidence_id_v1(
    ...
    raise SystemExit("C06-R000A stored snapshot did not re-resolve by immutable identity")
...
try:
    try:
        ...
        if exc.code != "OE_SNAPSHOT_WRITER_BUSY":
            raise SystemExit(f"C06-R0008 writer loser returned {exc.code}")
    else:
        raise SystemExit("C06-R0008 concurrent writer was admitted")
finally:
    writer_lock.rmdir()
if (snapshot_root / "CURRENT").read_bytes() != current_raw:
    raise SystemExit("C06-R0008 writer loser changed the winner selection")
...
try:
    ...
    if exc.code != "OE_JSON_MALFORMED":
        raise SystemExit(f"C06-R0006 invalid owner input returned {exc.code}")
else:
    raise SystemExit("C06-R0006 invalid owner input was published as DEGRADED")
if (invalid_run / "operational-evidence/snapshots/CURRENT").exists():
    raise SystemExit("C06-R0006 invalid owner input advanced CURRENT")
...
for index, fault_stage in enumerate(fault_stages, 300):
    ...
    try:
        ...
        if exc.code != expected_code:
            raise SystemExit(
                f"C06-R0007 {fault_stage} returned {exc.code}, expected {expected_code}")
    else:
        raise SystemExit(f"C06-R0007 {fault_stage} did not interrupt publication")
    current_path = fault_snapshots / "CURRENT"
    if fault_stage == "after-current-replace":
        current_value = fault_module._snapshot_current_raw_v1(fault_snapshots)
        if current_value is None:
            raise SystemExit("C06-R0007 post-replace fault lost the new complete selection")
    elif current_path.exists() or current_path.is_symlink():
        raise SystemExit(f"C06-R0007 {fault_stage} advanced CURRENT")
    ...
    if fault_snapshots.exists():
        ...
        if residue:
            raise SystemExit(f"C06-R0007 {fault_stage} left private temp residue: {residue}")
PY

if [ "${1:-}" = "--native-fixture-only" ]; then
  printf 'operational-evidence-contract.test: native fixture ok\n'
  exit 0
fi
