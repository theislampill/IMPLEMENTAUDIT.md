...
# 2. A manifest that returns to skills="./skills/" must fail.
...
The archive strips the `skills/` prefix.
EOF
if bash scripts/check-package-shape-claims.sh --scan-root "$tmp/bad-manifest" >/dev/null 2>&1; then
  printf 'package-shape-claims.test: expected stale manifest path to fail\n' >&2
  exit 1
fi
...
The package includes `skills/references/child-agents.md`.
The package includes `skills/scripts/claim-run.sh`.
Use the packaged templates under `skills/templates/` when available.
EOF
