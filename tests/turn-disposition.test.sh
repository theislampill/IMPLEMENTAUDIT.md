...
# Nullable wire regressions run before any real owner or temporary-store fixture.
wire_python=()
...
else printf 'turn-disposition.test: Python 3 is required\n' >&2; exit 1; fi
"${wire_python[@]}" -I -S -B - "$repo_root/skills/implementaudit/scripts/host-stop-interlock.py" <<'PY_STOP_WIRE'
import io
import json
import os
...
adapter = types.ModuleType('stop_wire_regression')
adapter.__file__ = str(path)
exec(compile(path.read_bytes(), str(path), 'exec'), adapter.__dict__)

def forbid(*args, **kwargs):
    raise AssertionError('real owner/effect path reached in Stop wire regression')
...
for name in ('run', 'validate_event', 'evaluate', 'current_proximal_selection', 'consume_proximal_selection'):
    setattr(adapter, name, forbid)

def invoke(raw, bound=False):
    out = io.StringIO()
    calls = []
    binding = {'schema': 'implementaudit.host-session-binding.v1', 'host_id': 'codex',
               'host_session_id': 'synthetic-wire-session', 'controller_id': 'synthetic-controller',
               'claim_id': 'synthetic-claim', 'explicit_run_root': 'synthetic-only',
               'repository_identity': 'synthetic-repo', 'git_common_directory_identity': 'synthetic-common',
               'worktree_identity': 'synthetic-worktree', 'binding_generation': 'G0001',
               'activation_event_id': 'synthetic-activation', 'activation_receipt': 'synthetic-activation-receipt',
               'applicable_continuity_generation': 'G0001', 'applicable_continuity_receipt': 'synthetic-receipt',
               'status': 'ACTIVE', 'predecessor_generation': None, 'supersession_or_tombstone_reason': None}
    def lookup(store, session, core):
        assert session == 'synthetic-wire-session'
        calls.append('lookup')
        return binding if bound else None
    adapter.plugin_store = lambda: calls.append('store_path_only') or Path('synthetic-only')
    adapter.owner_scripts = lambda: (None, None, None, None)
    adapter.lookup_binding = lookup
    adapter.current_route = lambda *a: calls.append('route') or {'decision': 'NOT_REQUIRED'}
...
base = {'session_id': 'synthetic-wire-session', 'turn_id': 'synthetic-turn', 'hook_event_name': 'Stop',
        'stop_hook_active': False, 'last_assistant_message': 'Synthetic ordinary text.',
        'transcript_path': None, 'cwd': 'synthetic-only', 'model': 'synthetic-model', 'permission_mode': 'default'}
cases = [
    ('null reentry takes zero-owner non-authorizing path', wire({**base, 'last_assistant_message': None, 'stop_hook_active': True}), True, 'reentry'),
    ('null unbound remains no-active-object', wire({**base, 'last_assistant_message': None}), False, 'unbound'),
    ('null bound stops without requesting continuation or closure', wire({**base, 'last_assistant_message': None}), True, 'unavailable'),
    ('nonnull reentry remains unchanged', wire({**base, 'stop_hook_active': True}), True, 'reentry'),
    ('nonnull unbound remains unchanged', wire(base), False, 'unbound'),
    ('standard bound event without trusted origin stays non-authorizing', wire(base), True, 'unknown-origin'),
    ('boolean assistant text refuses', wire({**base, 'last_assistant_message': False}), True, 'malformed'),
    ('empty assistant string retains existing refusal', wire({**base, 'last_assistant_message': ''}), True, 'malformed'),
    ('control-byte assistant string refuses', wire({**base, 'last_assistant_message': 'bad\x00text'}), True, 'malformed'),
    ('oversized assistant string refuses', wire({**base, 'last_assistant_message': 'x' * 70000}), True, 'malformed'),
    ('extra input field refuses', wire({**base, 'unexpected': True}), True, 'malformed'),
    ('malformed JSON refuses', b'{', True, 'malformed'),
    ('duplicate Stop member still refuses', b'{"session_id":"synthetic-wire-session","session_id":"synthetic-wire-session","turn_id":"synthetic-turn","hook_event_name":"Stop","stop_hook_active":true,"last_assistant_message":null}', True, 'malformed'),
]
observed = []
for label, raw, bound, want in cases:
    try:
        output, calls = invoke(raw, bound)
        nonauthorizing = output.get('host_activation_proven') is False and not output.get('object_closed', False)
        passed = nonauthorizing and (
            (want == 'reentry' and output.get('status') == 'REENTRY' and output.get('forced_exit_evidence') is False and 'decision' not in output and not calls)
            or (want == 'unbound' and output.get('status') == 'UNBOUND' and output.get('active_audit_object') is False and 'decision' not in output and calls == ['store_path_only', 'lookup'])
            or (want == 'unavailable' and output.get('status') == 'BLOCK' and output.get('continue') is False and isinstance(output.get('stopReason'), str) and bool(output['stopReason']) and 'decision' not in output and output.get('active_audit_object') is None and calls == ['store_path_only', 'lookup'])
            or (want == 'unknown-origin' and output.get('status') == 'BLOCK' and output.get('decision') == 'block' and calls == ['store_path_only', 'lookup', 'route'])
            or (want == 'malformed' and output.get('status') == 'BLOCK' and output.get('decision') == 'block' and not calls))
...
print(json.dumps({'consumer': 'turn-disposition.test.sh', 'scope': 'synthetic wire only',
...
# Existing v1 route presentation is optional metadata, never effect authority.
"${wire_python[@]}" -I -S -B - "$repo_root/skills/implementaudit/scripts/evaluate-turn-disposition.py" <<'PY_ROUTE_PRESENTATION'
import copy
import json
import os
from pathlib import Path
...
path = Path(sys.argv[1]).resolve()
evaluator = types.ModuleType('turn_presentation_regression')
evaluator.__file__ = str(path)
exec(compile(path.read_bytes(), str(path), 'exec'), evaluator.__dict__)

def audit(event, args):
    if event in ('subprocess.Popen', 'os.system', 'os.spawn', 'ctypes.dlopen') or event.startswith('socket.'):
        raise AssertionError('effect reached in pure route presentation controls')
    if event == 'open' and isinstance(args[2], int) and args[2] & (os.O_WRONLY | os.O_RDWR | os.O_CREAT | os.O_TRUNC | os.O_APPEND):
        raise AssertionError('write reached in pure route presentation controls')
sys.addaudithook(audit)

base = {'schema': 'implementaudit.route-transaction-result.v1', 'status': 'CURRENT',
    'decision': 'NOT_REQUIRED', 'classification': 'MECHANICALLY_NOT_REQUIRED',
    'advance_allowed': False, 'admission_required': True, 'enforcement_available': True,
    'record_oid': '1' * 40, 'record_identity': 'sha256:' + '2' * 64,
    'obligation_id': None, 'route_transaction_id': None, 'route_state': None,
    'selected_child': None, 'governor_decision_count': 0, 'history_query': None,
    'history_read_performed': False, 'mirror_claim': 'ABSENT', 'mirror_status': 'IGNORED_ABSENT',
    'projection_status': 'CURRENT', 'proof_layers': {'source_core': 'PRESENT', 'package': 'UNVERIFIED',
        'install': 'UNVERIFIED', 'host_activation': 'UNVERIFIED'}, 'host_activation_proven': False}
binding = {'host_id': 'codex', 'host_session_id': 'synthetic-presentation-session',
    'binding_generation': 'G0001', 'controller_id': 'synthetic-controller', 'claim_id': 'synthetic-claim',
    'explicit_run_root': 'synthetic-run', 'repository_identity': 'synthetic-repository',
    'git_common_directory_identity': 'synthetic-common', 'worktree_identity': 'synthetic-worktree',
    'applicable_continuity_generation': 'G0001',
    'applicable_continuity_receipt': 'refs/implementaudit/continuity-receipts/synthetic-controller/G0001@' + '3' * 40,
    'event_id': 'synthetic-event', 'turn_id': 'synthetic-turn', 'tool_use_id': None, 'agent_id': None,
    'obligation_id': None, 'route_transaction_id': None}
presentation = {'parent_holon': 'synthetic-parent', 'consuming_frontier': 'ordinary Stop continuation'}
current = dict(base, presentation=presentation)
required = dict(current, decision='REQUIRED', classification='MECHANICALLY_REQUIRED',
    advance_allowed=True, admission_required=False, route_state='SATISFIED',
    selected_child='audit-assess', governor_decision_count=1,
    obligation_id='synthetic-obligation', route_transaction_id='sha256:' + '4' * 64)
required_binding = dict(binding, obligation_id=required['obligation_id'], route_transaction_id=required['route_transaction_id'])
observed = []

def check(label, route, correlation, want):
    before = copy.deepcopy(route)
    try:
        result = evaluator.validate_route(route, correlation)
        passed = want == 'accept' and result == before and route == before
        observed.append({'case': label, 'passed': passed, 'expected': want,
            'actual': 'accept', 'complete_route_preserved': result == before, 'input_unchanged': route == before})
    except (evaluator.InputError, evaluator.DecisionBlocked) as exc:
        actual = type(exc).__name__
        observed.append({'case': label, 'passed': actual == want and route == before,
            'expected': want, 'actual': actual, 'reason': str(exc), 'input_unchanged': route == before})

check('historical v1 exact carrier remains readable without invented metadata', base, binding, 'accept')
check('current no-child presentation reaches evaluator unchanged', current, binding, 'accept')
check('satisfied REQUIRED preserves carried presentation', required, required_binding, 'accept')
legacy_required = copy.deepcopy(required); legacy_required.pop('presentation')
check('historical satisfied REQUIRED carrier remains readable', legacy_required, required_binding, 'accept')
check('canonical 240-character boundary', dict(current, presentation={'parent_holon': 'p' * 240, 'consuming_frontier': 'f' * 240}), binding, 'accept')
check('canonical Unicode labels are preserved', dict(current, presentation={'parent_holon': 'parent-é', 'consuming_frontier': 'frontier-λ'}), binding, 'accept')
check('labels are preserved as metadata rather than inferred authority', dict(current, presentation={'parent_holon': 'different-label', 'consuming_frontier': 'different-frontier'}), binding, 'accept')

for label, value in (
    ('null presentation', None), ('array presentation', []), ('empty presentation', {}),
    ('missing frontier', {'parent_holon': 'parent'}),
    ('unknown presentation authority key', dict(presentation, authority='ALLOW')),
    ('foreign presentation envelope', {'schema': 'other.presentation.v1', 'value': presentation})):
    check(label, dict(current, presentation=value), binding, 'InputError')
for key in ('parent_holon', 'consuming_frontier'):
    for label, text in (
        ('empty', ''), ('leading whitespace', ' x'), ('trailing whitespace', 'x '),
        ('control character', 'x\ny'), ('DEL', 'x\x7fy'), ('backtick', 'x`y'),
        ('241 characters', 'x' * 241), ('boolean', True), ('number', 1), ('null', None)):
        value = dict(presentation); value[key] = text
        check(key + ': ' + label, dict(current, presentation=value), binding, 'InputError')

check('unknown top-level field still refuses', dict(current, unexpected=True), binding, 'InputError')
missing = copy.deepcopy(current); missing.pop('record_identity')
check('missing required core field still refuses', missing, binding, 'InputError')
check('foreign route schema still blocks', dict(current, schema='other.route-result.v1'), binding, 'DecisionBlocked')
check('presentation does not permit stale route', dict(current, status='STALE'), binding, 'DecisionBlocked')
check('presentation does not permit PENDING', dict(current, decision='PENDING'), binding, 'DecisionBlocked')
check('presentation does not mint host activation', dict(current, host_activation_proven=True), binding, 'DecisionBlocked')
check('presentation does not bypass enforcement', dict(current, enforcement_available=False), binding, 'DecisionBlocked')
check('presentation does not authorize NOT_REQUIRED advance', dict(current, advance_allowed=True), binding, 'DecisionBlocked')
check('presentation does not satisfy an unsatisfied lifecycle', dict(required, route_state='UNSATISFIED'), required_binding, 'DecisionBlocked')
check('presentation does not replace foreign obligation custody', required, dict(required_binding, obligation_id='foreign-obligation'), 'DecisionBlocked')
check('presentation does not replace foreign transaction custody', required, dict(required_binding, route_transaction_id='sha256:' + '5' * 64), 'DecisionBlocked')
check('presentation does not bypass exactly-one governor decision', dict(required, governor_decision_count=2), required_binding, 'DecisionBlocked')
check('bad metadata never downgrades to the historical carrier', dict(base, presentation=None), binding, 'InputError')
print(json.dumps({'consumer': 'evaluate-turn-disposition.validate_route', 'scope': 'pure source carrier controls',
    'cases': observed, 'passed': sum(row['passed'] for row in observed), 'total': len(observed),
    'native_or_currentness_credit': False}, sort_keys=True))
if not all(row['passed'] for row in observed): raise SystemExit(1)
PY_ROUTE_PRESENTATION
...
if claim == "NO_ACTIVE_AUDIT_OBJECT":
    ...
    route = {
        ...
        "selected_child": "audit-assess" if obligation else None,
...
# This is a disposable source fixture with the same physical owner topology
# required by the unchanged isolated Stop handler. It is not a host install.
stop_plugin_root="$tmp/h/plugins/cache/m/p/v"
stop_plugin_data="$tmp/h/plugins/data/m/p"
mkdir -p "$stop_plugin_root" "$stop_plugin_data"
for member in skills .codex-plugin .claude-plugin hooks package; do
  cp -R "$repo_root/$member" "$stop_plugin_root/$member"
done
stop_scripts="$stop_plugin_root/skills/implementaudit/scripts"
host_stop_adapter="$stop_scripts/host-stop-interlock.py"
host_binding_core="$stop_scripts/host-session-binding.py"
route_core="$stop_scripts/route-transaction.py"
claim_helper="$stop_scripts/claim-run.sh"
cmp "$repo_root/skills/implementaudit/scripts/host-stop-interlock.py" "$host_stop_adapter"

stop_repo="$tmp/stop-repo"
stop_run_rel=".IMPLEMENTAUDIT/runs/stop-ABC123"
stop_run_shell="$stop_repo/$stop_run_rel"
stop_store="$stop_plugin_data/host-session-binding-v1"
stop_session="stop-session"
stop_controller="stop-controller"
stop_claim="99999999999999999999999999999999"
stop_logical_task="hc_h7b_maintainer_fixture"
mkdir -p "$stop_repo" "$stop_run_shell"
git -C "$stop_repo" init -q
git -C "$stop_repo" config core.longpaths true
...
value={
 ...
 "presentation":{"parent_holon":"HC-H7B-fixture","consuming_frontier":"ordinary Stop continuation"},
...
stop_event() {
  local turn="$1" reentry="$2" message="$3" session="${4:-$stop_session}"
  python - "$session" "$turn" "$reentry" "$message" "$stop_receipt" <<'PY'
import hashlib,json,sys
session,turn,reentry,message,receipt=sys.argv[1:]
event={
...
# Explicit synthetic ordinary classification, bound to this exact event.
# This fixture cannot establish a standard native host classification producer.
identity={"schema":"implementaudit.codex-stop-event.v1","host_id":"codex",
 "host_session_id":session,"turn_id":turn,"binding_generation":"G0001",
 "continuity_generation":"G0001","continuity_receipt":receipt,
 "last_assistant_message_digest":"sha256:"+hashlib.sha256(message.encode("utf-8")).hexdigest()}
event_id="codex-stop-v1-"+hashlib.sha256(json.dumps(identity,sort_keys=True,separators=(",",":")).encode()).hexdigest()
event["abnormality_cognition"]={"schema":"implementaudit.host-abnormality-classification.v1",
 "event_id":event_id,"classification":"NONE"}
print(json.dumps(event,sort_keys=True,separators=(",",":")))
PY
}

run_stop_hook() {
  local plugin_data="$1" payload="$2" output status plugin_root_native plugin_data_native
  plugin_root_native="$(python -c 'from pathlib import Path; import sys; print(Path(sys.argv[1]).resolve())' "$stop_plugin_root")"
  plugin_data_native="$(python -c 'from pathlib import Path; import sys; print(Path(sys.argv[1]).resolve())' "$plugin_data")"
  set +e
  output="$(cd "$tmp" && PLUGIN_ROOT="$plugin_root_native" PLUGIN_DATA="$plugin_data_native" \
    python -I -S -B "$host_stop_adapter" <<<"$payload" 2>"$tmp/stop-hook.err")"
  status=$?
  set -e
  [ "$status" -eq 0 ] || {
...
# The synthetic classified cases below must not mask the standard missing
# classification refusal after reaching the real owned store/route boundary.
unclassified_event="$(stop_event turn-unclassified false 'Synthetic event without a classification.')"
unclassified_event="$(python -c 'import json,sys; value=json.loads(sys.argv[1]); value.pop("abnormality_cognition"); print(json.dumps(value,separators=(",",":")))' "$unclassified_event")"
unclassified_result="$(run_stop_hook "$stop_plugin_data" "$unclassified_event")"
assert_stop_result "$unclassified_result" \
  'value["status"] == "BLOCK" and value["decision"] == "block" and "absent structured abnormality classification" in value["reason"]' \
  'missing classification reached an ordinary continuation'
...
missing_action_result="$(run_stop_hook "$stop_plugin_data" "$missing_action_event")"
...
false_terminal_result="$(run_stop_hook "$stop_plugin_data" "$false_terminal_event")"
...
progress_result="$(run_stop_hook "$stop_plugin_data" "$progress_event")"
...
duplicate_one="$(run_stop_hook "$stop_plugin_data" "$progress_event")"
duplicate_two="$(run_stop_hook "$stop_plugin_data" "$progress_event")"
...
copied_selection_result="$(run_stop_hook "$stop_plugin_data" \
...
unsatisfied_result="$(run_stop_hook "$stop_plugin_data" "$(stop_event turn-unsatisfied false 'Waiting without a satisfied governed route.')")"
...
opened="$(run_stop_route open --request "$required_route_request" --expected-record "$required_oid" \
  --packet "$route_packet" --logical-task "$stop_logical_task" 2>"$tmp/stop-open.visible")"
...
required_progress_result="$(run_stop_hook "$stop_plugin_data" \
...
unbound_data="$stop_plugin_data"
unbound_before="$(stop_snapshot)"
unbound_result="$(run_stop_hook "$unbound_data" "$(stop_event unbound-turn false 'Ordinary unbound turn.' unbound-stop-session)")"
...
[ "$(stop_snapshot)" = "$unbound_before" ] || {
  printf 'turn-disposition.test: unbound Stop changed the pre-existing physical owner store\n' >&2
  exit 1
}
...
for hostile_stop in \
  ...
  hostile_result="$(run_stop_hook "$stop_plugin_data" "$hostile_stop")"
...
oversized_result="$(run_stop_hook "$stop_plugin_data" "$oversized_stop")"
...
tombstoned_result="$(run_stop_hook "$stop_plugin_data" \
